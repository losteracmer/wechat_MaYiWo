...
router.use("/login", (Request, Response) => {
    ...
    let sql = `SELECT * FROM admins WHERE admins.account = '${account}'`;
...
router.use('/getoptions', (Request, Response) => {
    let sql = `SELECT * FROM admin_options WHERE show_status = 0`;
    mysql.query(sql).then(resset => {

        console.log('获取到adminoptions信息:', resset);
        Response.send({
            code: 200,
            adminoptions: resset
        })
    }).catch(error => {
        console.log('获取adminoptions发生错误', error)
    })
})
